...
function getName(): string
{
    ...
    $name = $firstName[mt_rand(0, count($firstName) - 1)] . $secondName[mt_rand(0, count($secondName) - 1)];
    if (mt_rand(0, 1) == 1) {
        $name .= $secondName[mt_rand(0, count($secondName) - 1)];
    }
    return $name;
}

function getValidateCode($number)
{
    $salt = [7,9,10,5,8,4,2,1,6,3,7,9,10,5,8,4,2];
    $vCode = [1, 0, 'X', 9, 8, 7, 6, 5, 4, 3, 2];
    $sum = 0;
    for ($i = 0; $i < 17; $i++) {
        $sum += substr($number, $i, 1) * $salt[$i];
    }
    return $vCode[$sum % 11];
}
...
function getCertNo(): string
{
    ...
    $certNo = $areaCode[mt_rand(0, count($areaCode) - 1)][0] . date('Ymd', mt_rand(0, 978192000)) . mt_rand(100, 999);
    return $certNo . getValidateCode($certNo);
}
...
{
    $bin = ['130', '131', '132', '133', '134', '135', '136', '137', '138', '139', '144', '147', '150', '151', '152', '153', '155', '156', '157', '158', '159', '176', '177', '178', '180', '181', '182', '183', '184', '185', '186', '187', '188', '189'];
    return $bin[mt_rand(0, count($bin) - 1)] . mt_rand('1111', '9999') . mt_rand('1111', '9999');
}
...
<form action="">
    <table>
        <tr>
            ...
            <th>银行卡号</th>
            <th>手机号</th>
            <th class="bank_info">银行名称</th>
            <th class="bank_info">银行编码</th>
        </tr>
        <?php for($i = 1; $i<= 5;$i++){ ?>
            <?php echo "<tr>" ?>
            <?php echo "<td>".$i."</td>" ?>
            <?php echo "<td>".getName()."</td>" ?>
            <?php echo "<td>".getCertNo()."</td>" ?>
            <?php echo "<td>".getCard()['card_no']."</td>" ?>
            <?php echo "<td>".getPhone()."</td>" ?>
            <?php echo "<td>".getCard()['name']."</td>" ?>
            <?php echo "<td>".getCard()['code']."</td>" ?>
        <?php } ?>
        <tr>
            <td colspan="8" id="colspan">
                <button>刷新</button>
<!--                <button type="button" id="show_bank_info">展示银行信息</button>-->
<!--                <button type="button" id="hide_bank_info">隐藏银行信息</button>-->
            </td>
        </tr>
